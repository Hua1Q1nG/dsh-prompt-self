		...
		function PromptSelfViewer({ t, compact }) {
			...
			const load = useCallback(async () => {
				try {
					...
					setState((s) => ({ ...s, status: "ready", profile, config, error: null }));
				} catch (error) {
			...
			return createElement("div", { style: containerStyle },
				...
				: profile && profile.ok !== false ? createElement(react.Fragment, null,
					...
					profile.updatedAt ? createElement("div", { style: itemMutedStyle }, tr("updatedAt") + " " + new Date(profile.updatedAt).toLocaleString())
						: null
				)
