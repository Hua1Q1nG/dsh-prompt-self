...
function mergeProfile(profileText, analysis, cfg) {
  ...
  if (entry !== undefined && entry !== null && typeof entry === "object") {
    ...
    parsed.records.unshift([header, ...lines]);
    parsed.records = parsed.records.slice(0, cfg.maxRecords);
  }
  return renderProfile(parsed);
}
...
function renderProfile(parsed) {
  ...
  blocks.push("", "## 学习记录", "<!-- 按时间倒序，由插件自动追加 -->");
...
function optimizeSystem(profile) {
  return [
...
async function handleProfileRoute(req, res, profilePath, configPath) {
  if (req.method !== "GET") {
...
function installWebRoutes(ctx, profilePath, configPath) {
  ctx.inject(["webServer"], (sctx) => {
    const server = sctx.get("webServer");
    sctx.effect(() => server.register({
      kind: "exact",
      path: "/prompt-self/profile",
      handler: (req, res) => handleProfileRoute(req, res, profilePath, configPath)
...
function apply(ctx, config = {}) {
  ...
  if (scopeOf(ctx) === undefined && cfg.forceEngine !== true) {
    ...
    installWebRoutes(ctx, profilePath, configPath);
    return;
  }
  if (scopeOf(ctx) === undefined && cfg.forceEngine === true) {
  ...
  async function learnOnce(session, force) {
    try {
      ...
      const merged = mergeProfile(profileText, analysis, cfg);
      await persist(merged, profilePath);
...
export { apply, name, cleanOutput, collectPair, handleConfigRoute, handleProfileRoute, hasRules, isFinCommand, latestUserMessageFromClaim, latestUserPrompt, mergeProfile, messageText, parseAnalysis, parseProfile, readRuntimeConfig, renderProfile };
